...
@section('title', 'Editar OT #' . $workOrder->folio_display)
...
<div class="animate-in">
    ...
    <div class="d-flex align-items-start justify-content-between mb-4">
        <div>
            ...
            <h2 class="page-title mb-0">Editar OT #{{ $workOrder->folio_display }}</h2>
        </div>
        <div class="d-flex gap-2 align-items-center pt-1">
            ...
            <button type="submit" form="workOrderForm" class="btn-primary-premium">
                <i class="bi bi-check-circle-fill"></i> Actualizar OT
            </button>
        </div>
    </div>
        ...
        <div class="row g-4 align-items-start">
            ...
            <div class="col-lg-4 d-flex flex-column gap-3">
                        ...
                        <div class="input-group">
                            <div class="ac-wrap">
                                <input type="text" id="client_text"
                                    ...
                                    value="{{ $workOrder->client->name . ' — ' . $workOrder->client->rut_dni }}">
                                <input type="hidden" name="client_id" id="client_id" value="{{ $workOrder->client_id }}">
                            </div>
                        ...
                        <div class="input-group">
                            <div class="ac-wrap">
                                <input type="text" id="vehicle_text"
                                    ...
                                    value="{{ $workOrder->vehicle->license_plate . ' — ' . $workOrder->vehicle->brand . ' ' . $workOrder->vehicle->model }}">
                                <input type="hidden" name="vehicle_id" id="vehicle_id" value="{{ $workOrder->vehicle_id }}">
                            </div>
                ...
                <div class="card p-4">
                            ...
                            <select name="insurance_company_id" id="insurance_company_id" class="form-select">
                                ...
                                @foreach($insuranceCompanies as $company)
                                    <option value="{{ $company->id }}"
                                        {{ $workOrder->insurance_company_id == $company->id ? 'selected' : '' }}>
                                        {{ $company->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="input-group">
                            <select name="liquidator_id" id="liquidator_id" class="form-select">
                                ...
                                @foreach($liquidators as $liq)
                                    <option value="{{ $liq->id }}"
                                        {{ $workOrder->liquidator_id == $liq->id ? 'selected' : '' }}>
                                        {{ $liq->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-icon-wrap">
                            ...
                            <input type="number" name="deductible_amount" class="form-control"
                                value="{{ $workOrder->deductible_amount }}" min="0">
                        </div>
                    </div>
                    ...
                    <div class="mb-0">
                        ...
                        <textarea name="notes" class="form-control" rows="3"
                            placeholder="Notas adicionales…">{{ $workOrder->notes }}</textarea>
                    </div>
                </div>
                ...
                <div class="card p-4">
                    ...
                    @php $selectedTags = $workOrder->tags->pluck('id')->toArray(); @endphp
                    @foreach($tags as $tag)
...
@foreach($workOrder->items as $item)
    addRow({
        ...
        price_workshop: {{ $item->price_workshop }},
        price_authorized: {{ $item->price_authorized }},
        price_real: {{ $item->price_real }},
        is_approved: {{ $item->is_approved ? 'true' : 'false' }},
        is_salvage: {{ $item->is_salvage ? 'true' : 'false' }}
    });
@endforeach
